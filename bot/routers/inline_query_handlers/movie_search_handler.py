...
async def inline_query_title(inline_query: InlineQuery) -> None:
    ...
    items = map(lambda m: InlineQueryResultArticle(id=str(m['filmId']), title=m['nameRu'] if m['nameRu'] else m['nameEn'], input_message_content=InputTextMessageContent(message_text="content"), description=f"🌟 {m['rating']} 📼 Год: {m['year']} 🔸 {m['genres'][0]['genre']}", thumbnail_url=m['posterUrl']) , MovieService.get_movie_by_title(title)['films'])
