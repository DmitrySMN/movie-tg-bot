...
async def handle_premiere_button_click(callback: CallbackQuery):
    ...
    await callback.bot.send_photo(chat_id=callback.message.chat.id, photo=movie["posterUrl"], caption=f'💥{markdown.bold(movie['nameRu'])}', reply_markup=get_addition_keyboard())
    await callback.answer()